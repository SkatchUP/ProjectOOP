...
def create_food():
    ...
    do('''CREATE TABLE IF NOT EXISTS foods(
...
def check_id_from_log_pas(login, password):
    open()
...
def change_imt(user_id, update_imt):
    open()
    cursor.execute('UPDATE imt_users SET imt=? WHERE id_imt=?', [update_imt, user_id])
    conn.commit()
